                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped datatables" id="example">
                                ...
                                <tbody id="list-data">
                                    <?php foreach ($users as $users) { ?>
                                        <tr class="<?php if($users->u_group=='2') echo 'sup-admin-row'?>">
                                            ...
                                            <td><span class="<?php if($users->u_group=='2') echo 'sup-admin-name' ?>"> <?php echo ($users->u_group=='2')? 'City Admin':'Normal User'; ?></span></td>
                                            <td><?php echo ($users->u_status == 0) ? "Inactive" : "Active"; ?></td>
                                            <td class="center worker-btn">
                                               <?php  if($users->u_status == 0){?>
                                                <a  class='btn btn-success' href='javascript:;' onclick='approval(<?php echo $users->u_id ?>, 1)'>Active</a>
                                               <?php }else{?>
...
function approval(id, val) {
    var form_data = {
        ...
        value: val};
    $.ajax({
        url: '<?php echo site_url() . 'admin/users/accesspermission'; ?>',
        data: form_data,
        type: 'post',
        datatype: 'json',
